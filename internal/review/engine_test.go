...
// A generated file whose patch GitHub omitted is still covered: it is a
// deterministic output of a reviewed source, so it must not lower confidence.
func TestOmittedPatchesExemptsDerivedFiles(t *testing.T) {
	head, base := t.TempDir(), t.TempDir()
	writeTestFile(t, filepath.Join(head, "gen", "model.pb.go"), "// Code generated by protoc-gen-go. DO NOT EDIT.\npackage gen\n")
	writeTestFile(t, filepath.Join(head, "internal", "hand.go"), "package internal\n")
	writeTestFile(t, filepath.Join(base, "old", "gone.pb.go"), "// Code generated by protoc-gen-go. DO NOT EDIT.\n")
	files := []report.ChangedFile{
		{Filename: "gen/model.pb.go", Status: "modified", Additions: 3, Deletions: 3},
		{Filename: "internal/hand.go", Status: "modified", Additions: 2, Deletions: 2},
		{Filename: "old/gone.pb.go", Status: "removed", Additions: 0, Deletions: 4},
	}
	gaps, err := OmittedPatches(head, base, files, []string{"gen/model.pb.go", "internal/hand.go", "old/gone.pb.go"})
	if err != nil {
		t.Fatal(err)
	}
	want := []string{"GitHub omitted the patch for internal/hand.go"}
	if !reflect.DeepEqual(gaps, want) {
		t.Fatalf("gaps = %v, want %v", gaps, want)
	}
}

func TestOmittedPatchesExemptsGeneratedNames(t *testing.T) {
	head, base := t.TempDir(), t.TempDir()
	writeTestFile(t, filepath.Join(head, "a", "types.pb.go"), "package a\n")
	gaps, err := OmittedPatches(head, base, []report.ChangedFile{{Filename: "a/types.pb.go", Status: "modified"}}, []string{"a/types.pb.go"})
	if err != nil || len(gaps) != 0 {
		t.Fatalf("gaps = %v, err = %v", gaps, err)
	}
}

func TestOmittedPatchesExemptsGeneratedBanner(t *testing.T) {
	head, base := t.TempDir(), t.TempDir()
	writeTestFile(t, filepath.Join(head, "schema", "dump.txt"), "# Automatically generated by tooling\n")
	gaps, err := OmittedPatches(head, base, []report.ChangedFile{{Filename: "schema/dump.txt", Status: "modified"}}, []string{"schema/dump.txt"})
	if err != nil || len(gaps) != 0 {
		t.Fatalf("gaps = %v, err = %v", gaps, err)
	}
}

func TestDerivedOmittedPatchKeepsConfidence(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, filepath.Join(dir, "head", "api", "x.pb.go"), "// Code generated by protoc-gen-go. DO NOT EDIT.\npackage api\n")
	if err := os.MkdirAll(filepath.Join(dir, "base"), 0700); err != nil {
		t.Fatal(err)
	}
	files := []report.ChangedFile{
		changedFiles[0],
		{Filename: "api/x.pb.go", Status: "modified", Additions: 3, Deletions: 3},
	}
	r, err := (Engine{config.Default(), &scriptedRunner{}}).Run(context.Background(), dir, Context{}, files)
	if err != nil {
		t.Fatal(err)
	}
	if *r.Confidence != 4 || !r.MergeReady() {
		t.Fatalf("derived omitted patch lowered confidence: %+v", r)
	}
}

func TestHandwrittenOmittedPatchLowersConfidence(t *testing.T) {
	dir := t.TempDir()
	writeTestFile(t, filepath.Join(dir, "head", "internal", "hand.go"), "package internal\n")
	if err := os.MkdirAll(filepath.Join(dir, "base"), 0700); err != nil {
		t.Fatal(err)
	}
	files := []report.ChangedFile{
	...
	if *r.Confidence != 3 || r.MergeReady() || !strings.Contains(strings.Join(r.Reasons, " "), "coverage limitations") {
		t.Fatalf("hand-written omitted patch did not lower confidence: %+v", r)
	}
}
